...
export default function ActiveJobsPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10 pb-20 pt-4 text-white">
      {/* Header */}
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}>
        <p className="text-xs font-mono font-bold text-[#D4AF37] uppercase tracking-widest mb-1">Live</p>
        <h1 className="text-4xl font-black tracking-tighter text-white">My Active Jobs</h1>
        <p className="text-gray-400 font-medium mt-1">Update job status as you work — clients see it in real time.</p>
      </motion.div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {[
          ...
          <motion.div
            ...
            className="card-luxury p-5 rounded-2xl"
          >
      ...
      {activeJobs.length === 0 && doneJobs.length === 0 ? (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="card-luxury rounded-2xl p-16 text-center space-y-4 border border-white/5">
          <Briefcase className="w-12 h-12 text-gray-500 mx-auto" />
          <h3 className="text-xl font-black text-white">No active jobs yet</h3>
        ...
        <>
                ...
                {activeJobs.map((job, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="card-luxury rounded-2xl p-6 md:p-8 space-y-5 border border-white/5"
                    >
                      {/* Top row */}
                      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                        <div className="flex-1 space-y-1">
                          ...
                          <h3 className="text-xl font-black text-white leading-tight">{job.title}</h3>
                        ...
                        <div className="text-right flex-shrink-0">
                          <p className="text-3xl font-black text-white">R {(job.price || 0).toLocaleString()}</p>
                      ...
                      {job.client && (
                        <div className="flex items-center gap-3 pt-2 border-t border-white/5">
                          ...
                          <div>
                            ...
                            <p className="text-sm font-bold text-white">{job.client.full_name}</p>
                          </div>
                        </div>
                      )}
          ...
          {doneJobs.length > 0 && (
            <section className="space-y-4">
              ...
              <div className="card-luxury rounded-2xl divide-y divide-white/5 overflow-hidden border border-white/5">
                {doneJobs.map(job => (

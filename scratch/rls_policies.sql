...
DROP POLICY IF EXISTS "Workers can view own quotes and clients can view quotes for their jobs" ON quotes;
...
DROP POLICY IF EXISTS "Clients can view own saved workers" ON saved_workers;
...
DROP POLICY IF EXISTS "Clients can insert saved workers" ON saved_workers;
...
DROP POLICY IF EXISTS "Clients can delete own saved workers" ON saved_workers;
...
DROP POLICY IF EXISTS "Worker portfolios are viewable by everyone" ON worker_portfolios;
...
DROP POLICY IF EXISTS "Workers can insert own portfolio items" ON worker_portfolios;
CREATE POLICY "Workers can insert own portfolio items" 
ON worker_portfolios FOR INSERT WITH CHECK (auth.uid() = worker_id);
DROP POLICY IF EXISTS "Workers can update own portfolio items" ON worker_portfolios;
CREATE POLICY "Workers can update own portfolio items" 
ON worker_portfolios FOR UPDATE USING (auth.uid() = worker_id);
DROP POLICY IF EXISTS "Workers can delete own portfolio items" ON worker_portfolios;

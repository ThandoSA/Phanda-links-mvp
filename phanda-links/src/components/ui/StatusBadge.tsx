export default function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`status-badge status-${normalized}`}>
      ...
      {normalized === "pending" && <span className="w-1.5 h-1.5 rounded-full bg-yellow-400 animate-pulse inline-block" />}
      {normalized === "rejected" && <span className="w-1.5 h-1.5 rounded-full bg-red-400 inline-block" />}
      {normalized === "completed" && <span className="w-1.5 h-1.5 rounded-full bg-gray-400 inline-block" />}
